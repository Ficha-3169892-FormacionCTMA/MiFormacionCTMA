...
## 5. 📱 Semana 9: Capacidades del Dispositivo y Seguridad

En este incremento se implementó la gestión de evidencias fotográficas para las actividades de **Mi Formación CTMA**, aplicando el principio de mínimo privilegio, almacenamiento seguro, validación estricta de archivos y resiliencia ante fallos de red.

---

### 🏛️ Arquitectura de Evidencias y Almacenamiento

* **Fuente Local Canónica (Room):** La entidad `EvidenciaEntity` persiste la referencia de la imagen (`localUri`), metadatos y su estado de sincronización. Jamás se almacenan binarios completos (`Bitmap` o `Base64`) dentro de la base de datos para evitar degradación de rendimiento y errores de memoria.
* **Separación de Responsabilidades:** La capa de interfaz (UI) maneja exclusivamente contratos de selección (`content://` URIs), delegando las comprobaciones de tamaño, tipo MIME y almacenamiento interno al `Repository` y `FileHelper`.

---

### 🛡️ Mínimo Privilegio y Captura Segura

* **Photo Picker (`PickVisualMedia`):** Utilizado para la selección de imágenes desde la galería sin requerir permisos de lectura global (`READ_MEDIA_IMAGES`).
* **FileProvider & Cámara:** La captura mediante la app de cámara utiliza `ActivityResultContracts.TakePicture()` vinculada a una `content://` URI generada por `FileProvider`. El acceso está restringido de forma exclusiva al subdirectorio privado `files/evidencias/`.
* **Notificaciones Contextuales:** La solicitud del permiso `POST_NOTIFICATIONS` (en Android 13+) se realiza únicamente cuando la persona activa explícitamente los recordatorios en la UI, evitando solicitudes invasivas al inicio de la aplicación.

---

### 🔄 Modelo de Estados de Sincronización

La evidencia transita por cuatro estados clave manejados por el `SyncedActividadRepository`:

1. **`LOCAL`:** Guardada en el almacenamiento interno del dispositivo y registrada en Room.
2. **`SUBIENDO`:** Operación de envío `Multipart` en curso hacia la API REST.
3. **`SINCRONIZADA`:** Recepción y confirmación exitosa por parte del servidor por HTTPS.
4. **`FALLIDA`:** Ocurrió un fallo de red o tiempo de espera. La evidencia local se preserva intacta para permitir reintentos sin pérdida de datos.

---

### 🔒 Validaciones y Seguridad en Red

* **Validaciones de Entrada:** Antes de persistir o enviar cualquier archivo, se valida que el tipo MIME sea compatible (`image/jpeg`, `image/png`, `image/webp`), que el tamaño no supere el límite máximo de 5 MB y que el stream de datos sea legible.
* **Ambientes de Compilación (`Build Variants`):**
  * **`dev` / `stage`:** Entornos de desarrollo y pruebas controladas con URLs específicas.
  * **`prodRelease`:** Tráfico forzado sobre HTTPS con `cleartextTrafficPermitted="false"`. Logs de HTTP desactivados y redactados para evitar la filtración de tokens `Authorization` o URIs sensibles en Logcat.

---

## 6. Implementaciones Detalladas (Semana 7)
